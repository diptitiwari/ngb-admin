      ...
      <vx-card title no-shadow card-border>
        <div class="vx-row">
          <div class="vx-col w-full">
            <div class="flex items-end px-3">
              <span class="font-medium text-lg leading-none">{{$t('label.createorganization')}}</span>
            </div>
            <vs-divider />
          </div>
        </div>
        <div class="vx-row">
          <div class="vx-col w-full lg:w-1/2 md:w-1/2 sm:w-full xs:w-full">
            <small class=" date-label font-semibold">{{$t('label.orgopenedon')}}</small>
            ...
            <small class="date-label">{{$t('label.Commission Model')}}</small>
            <v-select
              label="item"
              v-model="entity.commisionModel"
              ...
              placeholder="--select Search--"
              :dir="$vs.rtl ? 'rtl' : 'ltr'"
              class="min-w-200 w-full mb-2 font-semibold"
            >
          ...
          <div class="vx-col w-full lg:w-1/2 md:w-1/2 sm:w-full xs:w-full">
            ...
            <small class="date-label font-semibold">{{$t('label.segmentType')}}</small>
            <v-select
              label="name"
              v-model="entity.dasType"
            ...
            <vs-upload
              ...
              @change="this.profileUrl = this.$refs.docFile.files[0];"
            />
          </div>
        </div>
        <div class="vx-row float-right my-2">
          <div class="vx-col w-full">
            <vs-button class="mr-3 mb-2" @click="createEntity">{{$t('label.button.submit')}}</vs-button>
            <vs-button
              color="warning"
              type="border"
              class="mb-2"
...
export default {
  ...
  methods: {
    ...
    async createEntity() {
      ...
      try {
        const resp = await createEntity(this.entity);
        if (resp.data.officeId && resp.data.resourceId) {
          console.log("data send", this.entity);
  ...
  created() {
    this.getTemplate();
     this.entity.externalId = this.entity.officeType + Math.floor(Math.random()*1000000000);
  }
};
</script>
